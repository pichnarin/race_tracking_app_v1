...
class _RaceTestScreenState extends State<RaceTestScreen> {
  ...
  Future<void> _createRace() async {
    ...
    final raceName = "Cool Race"; // Example race name
      ...
      body: Padding(
        ...
        child: ListView(
          children: [
            ...
            // Button to trigger addParticipant using the entered bib and selected raceId
            ElevatedButton(
              onPressed: () async {
                if (_bib.isNotEmpty && _selectedRaceId != null) {
                  try {
                    final participant = await repo.addParticipant(
                      ...
                      segmentFinishTimes: {}, // Empty map for finish times
                      totalTime: '00:00:00', // Default total time
                    );
                    showLog('Participant added with PID: ${participant.pid}');
                  } catch (e) {
                    showLog('Failed to add test participant: $e');
                  }
                } else {
                  showLog('Please provide a bib number and select a race.');
                }
              },
